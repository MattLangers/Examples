...
{
    public class DatabaseSearchOrchestrator_SearchProducts_By_Name_With_Match_Tests
    {
        ...
        public void Setup()
        {
            ...
            autoMocker.Use<ISearchProductSpecificationFactory>(m => m.CreateSearchByName(It.Is<ProductSearchInputModel>(p => p == productSearchInputModel)) == new SearchProductByNameSpecification(productSearchInputModel));

            autoMocker.GetMock<IProductsToDtoMapper>().Setup(m => m.Map(It.Is<IQueryable<Product>>(m => m.Contains(product)))).Returns(expectedResult);
