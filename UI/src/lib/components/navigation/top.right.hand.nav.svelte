<script lang="ts">
    ...
    let navs = new NavigationHelper().navigation.filter(n => n.type == NavigationTypeEnum.TopRight)
</script>

<nav class="min-w-full flex items-center">
    <Hamburger bind:open={sidebar} />
    <div class="hide_on_small_screens">
        {#each navs as navigation}
            <a href={navigation.href} target="_blank" class="mr-1">
                {#if navigation.image === NavigationImageEnum.Github}
